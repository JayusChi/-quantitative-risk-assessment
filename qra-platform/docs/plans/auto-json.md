...
状态：九江真实资料转换内部功能验证完成；正式 G1 延后至工程验证/发布范围

更新时间：2026-08-26

### 真实资料验收进度（2026-08-26）

已为九江支线 `GDBZYQ-JJ-1` 建立 `jiangxi-natural-gas.jiujiang.v1` v1.0.0 专用映射，完成 65 个真实原始文件及 1 个派生对比 JSON 的盘点。映射能够解析两级表头、按管线编号排除相邻支线、受控地将同一台账映射为多个业务视图，并显式处理带单位后缀的数值。

黄金候选严格采用台账直接证明的 JJ001（0+000）至 JJ041G（10+938）全线边界，形成 1 个 10.938 km 管段。原 11 个照片变化点工作段没有权威里程或坐标，只作为差异对比保留；空间人口资料不足，因此不生成 `population_cells`，也不猜测坐标、昼夜分布或室内外比例。

命令行重复转换、真实 HTTP 网页转换和不可变快照 payload 深度一致，业务 JSON 哈希均为 `0da428e547120043b9a90a13608272f96573940f302999ee5cd297d7dbae965e`；倒置里程负向案例被阻断且未创建快照。脱敏黄金回归已纳入测试，架构检查和 106 项自动化测试通过。当前范围状态为 `PASSED_INTERNAL_FUNCTION_VALIDATION`。根据项目方 2026-08-26 决定，本阶段只验证功能是否实现，业务数据负责人签批暂不执行；正式 G1 记为 `DEFERRED_NOT_IN_CURRENT_FUNCTION_VALIDATION_SCOPE`，待工程验证或正式发布前恢复。完整证据见 `docs/project/stage1/README.md`。
...
该阶段的结构化转换功能验证已由九江真实案例覆盖。业务数据负责人对黄金候选忠实性的正式确认延后至工程验证或发布前；如取得权威桩号、坐标和人口空间资料，应创建新版本输入。不得用现有照片变化点或自由文本猜测可计算人口单元。
