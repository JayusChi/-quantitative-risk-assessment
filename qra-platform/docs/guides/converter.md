...
3. 状态为 `BLOCKED` 或 `READY_FOR_CONFIRMATION` 时点击“打开复核工作台”；
4. 在三栏页面比较同一业务字段的候选和原始证据，执行接受、手工修改、驳回、不适用或请求重新提取；
5. 门禁通过后填写快照名称、确认人和原因，原子创建不可变快照，可选择立即进入现有计算队列。

管理页面不再显示或要求粘贴 `review_decisions JSON`。详细流程见
...
首次运行如存在冲突或低置信度内容，状态为 `BLOCKED`。管理页面应使用人工复核工作台。
以下决定文件参数只作为命令行回归和开发兼容入口保留，不是普通业务默认流程：
...
旧命令行复核决定必须引用稳定 `review_id`，并保留复核人、含时区时间和原因：
