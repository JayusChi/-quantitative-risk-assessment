...
九江真实资料专用配置为 `jiangxi-natural-gas/jiangxi-natural-gas.jiujiang.v1.json`。它不继承通用配置，因为客户台账使用两层合并表头并把管线、管段、高后果区、完整性和管理措施放在同一张表中。配置通过精确行过滤限定 `GDBZYQ-JJ-1`，并显式允许一张已识别源表映射到多个业务目标；历史 11 段照片比例里程和无空间证据的人口假设不在该配置中生成。
